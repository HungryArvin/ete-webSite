<template>
  ...
  <div v-for="(item,$index) in questionList" :key="item.id" class="text item">
     ...
     <span class="answer">{{item.answer === null ? "学生暂未回答...." : item.answer }} {{item.answer == null || isNaN(item.answer) ? "" : "(分)"}}</span>
...
<script>
import {cheackStduentAnswer} from '@/api/question'
    ...
    methods: {
     init() {
         if(this.$route.params && this.$route.params.id !== ":id"){
             cheackStduentAnswer(this.$route.params.id).then(response => {
